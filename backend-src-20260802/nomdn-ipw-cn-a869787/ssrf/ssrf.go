...
// SetDNSServer keeps SSRF validation on the same resolver as the network
// probes. An empty value intentionally keeps the built-in public resolver.
...
func lookupIP(ctx context.Context, host string) ([]net.IP, error) {
	...
	server := dnsServer
	dnsMu.RUnlock()
	servers := []string{server, "223.5.5.5:53", "1.1.1.1:53", "119.28.28.28:53"}
	...
	for _, candidate := range servers {
		...
		resolver := &net.Resolver{
			...
			Dial: func(dialCtx context.Context, _, _ string) (net.Conn, error) {
				...
				return dialer.DialContext(dialCtx, "udp", candidate)
			},
		}
	...
	if lastErr == nil {
		lastErr = fmt.Errorf("no IP address found for %s", host)
	}
	return nil, lastErr
}
...
// ValidatedTarget contains the exact hostname and addresses checked by the
// SSRF validator. The hostname binding prevents a redirect to another domain
// from accidentally reusing the first request's resolved address.
...
func ValidateOutboundTarget(ctx context.Context, targetURL string) (context.Context, error) {
	...
	return context.WithValue(ctx, validatedIPsKey, ValidatedTarget{Host: strings.TrimSuffix(strings.ToLower(host), "."), IPs: ips}), nil
}
...
func SecureCheckRedirect(req *http.Request, via []*http.Request) error {
	...
	for _, r := range via {
		...
		ips, err := lookupIP(req.Context(), host)
		if err != nil {
			return err
		}
		for _, ip := range ips {
