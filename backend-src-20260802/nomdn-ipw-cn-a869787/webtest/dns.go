...
func dnsServers() []string {
	...
	preferred := dnsServer
	dnsMu.RUnlock()
	return []string{preferred, "223.5.5.5:53", "1.1.1.1:53", "119.28.28.28:53"}
}

func exchangeDNS(msg *dns.Msg) (*dns.Msg, time.Duration, error) {
	...
	for _, server := range dnsServers() {
		...
		response, duration, err := client.Exchange(msg, server)
...
func ResolveARecord(domain string) (DNSResult, error) {
	...
	response, _, err := exchangeDNS(msg)
